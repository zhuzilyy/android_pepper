...
//        }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        for (int dayNumber = leftDaysWithGaps + 1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            //zyfadd
            Paint p0, p1, p2 = mHeaderTextPaint;
            p0 = new Paint(Paint.ANTI_ALIAS_FLAG);
            p0.setColor(Color.parseColor("#dbdbdb"));
            p1 = new Paint(Paint.ANTI_ALIAS_FLAG);
            p1.setColor(Color.parseColor("#f2fcc7"));
//            p1.setStyle(Paint.Style.STROKE);
            p2.setStyle(Paint.Style.STROKE);
            String daytext = getDateTimeInterpreter().interpretDay(day);
            if (sameDay) {
                canvas.drawCircle(startPixel + mWidthPerDay / 2, mHeaderTextHeight / 2 + mHeaderRowPadding / 2 + (mAllDayEventHeight + mHeaderMarginBottom), mHeaderHeight / 2 - mHeaderRowPadding / 2, p0);
                canvas.drawCircle(startPixel + mWidthPerDay / 2, mHeaderTextHeight / 2 + mHeaderRowPadding / 2 + (mAllDayEventHeight + mHeaderMarginBottom), mHeaderHeight / 2 - mHeaderRowPadding / 2 - 5, p1);
            }
            canvas.drawText(daytext, startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding /2 + (mAllDayEventHeight + mHeaderMarginBottom), mHeaderTextPaint);
            //addend
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_UP && !mIsZooming && mCurrentScrollDirection == Direction.NONE) {
